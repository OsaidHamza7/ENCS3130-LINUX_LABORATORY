	...
	if [ $min -eq $max ];then # doing this to avoid a division by zero
	   ...
	   echo "1:0"
           ...
           for c in `cat uniqe.txt`
             do
		if [ $value -ge 0 ];then
		new=$(printf "%.2f" $(echo "scale=1; ($c - $min) / $result" | bc  )) #MinMax scaling,  scale 1 to print 1 digit after dot
                    if [ $new == "0.00" -o $new == "1.00" ];then
...
if [ $savedValue -eq 1 ] # Verify that the processed dataset is saved
   then
       echo -e "\e[91mAre you sure you want to exit?\e[0m"
else
       echo -e "\e[91mThe processed dataset is not saved.Are you sure you want to exit?\e[0m"
fi
